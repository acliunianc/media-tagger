...
export default function ScanDialog({ open: isOpen, onClose, onScan, scanning }: ScanDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay/60">
      <div className="bg-surface-light rounded-xl shadow-2xl w-[520px] max-h-[80vh] flex flex-col border border-border-strong">
        ...
        <div className="p-4 space-y-4 overflow-y-auto flex-1">
          <div>
            <label className="text-sm text-fg-subtle mb-2 block">扫描目录</label>
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {paths.map((p) => (
                <div
                  key={p}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg bg-input/60 text-sm"
                >
                  <FolderOpen className="w-4 h-4 text-fg-subtle shrink-0" />
              ...
              {paths.length === 0 && (
                <p className="text-sm text-fg-muted py-4 text-center">尚未选择目录</p>
              )}
            </div>
            <button
              onClick={handlePickFolder}
              className="mt-2 flex items-center gap-2 px-4 py-2 rounded-lg bg-elevated hover:bg-hover-strong text-sm transition-colors"
            >
          ...
          <div>
            <label className="text-sm text-fg-subtle mb-2 block">排除规则（每行一条，支持通配符）</label>
            <textarea
              value={excludeInput}
              onChange={(e) => setExcludeInput(e.target.value)}
              rows={4}
              className="w-full px-3 py-2 rounded-lg bg-input/60 border border-border-strong text-sm font-mono focus:outline-none focus:border-accent/50 resize-none"
              placeholder="*.tmp&#10;node_modules/"
            />
            <div className="flex gap-2 mt-2">
              <input
                ...
                placeholder="添加排除规则..."
                className="flex-1 px-3 py-1.5 rounded-lg bg-input/60 border border-border-strong text-sm focus:outline-none focus:border-accent/50"
              />
              <button
                onClick={handleAddExclude}
                className="px-3 py-1.5 rounded-lg bg-elevated hover:bg-hover-strong transition-colors"
              >

...
export default function DetailPanel({ file, onTagsChanged }: DetailPanelProps) {
  ...
  if (!file) {
    return (
      <div className="w-96 border-l border-border flex items-center justify-center text-fg-muted">
  ...
  return (
    <div className="w-96 border-l border-border flex flex-col overflow-hidden">
      <div className="p-4 border-b border-border">
        ...
        {file.status === 0 && (
          <span className="inline-block mt-1 text-xs px-2 py-0.5 rounded bg-warning/20 text-warning">
            文件已丢失
          </span>
        )}
      </div>
      ...
      <div className="p-4 flex-1 overflow-y-auto">
        <h4 className="text-xs font-medium text-fg-subtle uppercase tracking-wider mb-3">标签</h4>
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          {file.tags.length === 0 && (
            <span className="text-xs text-fg-muted">暂无标签</span>
          )}
        </div>
        ...
        <div className="relative">
          <input
            ...
            placeholder="添加标签..."
            className="w-full px-3 py-2 rounded-lg bg-input/60 border border-border-strong text-sm focus:outline-none focus:border-accent/50"
          />
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute z-10 top-full left-0 right-0 mt-1 bg-elevated border border-border-strong rounded-lg shadow-xl overflow-hidden">
              {suggestions.map((s) => (
                <button
                  key={s}
                  className="w-full text-left px-3 py-2 text-sm hover:bg-hover-strong transition-colors"
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div
          className={`text-fg-secondary ${truncate ? "truncate" : ""}`}
          title={title || value}
        >
          {value}
        </div>
